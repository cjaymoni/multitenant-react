  ...
  const loadLazyData = () => {
    ...
    loadLazyTimeout = setTimeout(() => {
      if (
        ...
        props.location.pathname === "/recommendation"
      ) {
        const fetcht = props.fetchFunction1;
        LoadFunction(fetcht);
      } else if (
        JSON.parse(localStorage.role) === "Head of Department" &&
        props.location.pathname === "/requestlist"
      ) {
        const fetcht = props.fetchFunction1;
        LoadFunction(fetcht);
      } else if (
        ...
        props.location.pathname === "/inventoryinfo" &&
