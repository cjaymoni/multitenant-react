...
export class SidebarDemo extends Component {
  constructor(props) {
    ...
    this.state = {
      visibleLeft: true,
      lastActiveIndexString: localStorage.getItem(""),
      lastActiveIndex: Number(this.state.lastActiveIndexString),
  ...
  componentDidMount() {
    const activeItem = SidebarItems.findIndex(
      (item) =>
        this.getPath(item.route) === this.getPath(this.props.location.pathname)
    );
    this.changeActiveIndex(activeItem);
  }
        ...
        <div className="w-2">
          ...
          >
            ...
            {SidebarItems.map((item, index) => {
              return (
                <Link to={item.route} className="no-underline p-2">
                  <div
